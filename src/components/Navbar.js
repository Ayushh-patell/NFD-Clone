...
const Navbar = () => {
  ...
  useGSAP(
    () => {
      ...
      const menu = Array.from(nav.firstChild.querySelector(".menu").children);
      ...
      gsap.to(nav, {
        backgroundColor: "rgba(55, 49, 69, 0.8)",
        scrollTrigger: {
          trigger: nav,
          start: "bottom 60px", // Change this value to adjust when the animation should start
          end: "bottom 0%",
          scrub: true, // Smoothing effect during scroll
        },
      });
      gsap.to(".nav_inner", {
      ...
      return () => {
        gsap.to(nav, { clearProps: "all" }); // Clear inline styles
      };
    },
    { scope: navRef }
  );
  window.onscroll = function (e) {
    ...
    if (this.oldScroll > this.scrollY) {
      smlNav.classList.add("showNav");
    } else {
      smlNav.classList.remove("showNav");
    }
  ...
  return (
    <>
      <nav
        ref={navRef}
        className=" fixed px-7 py-4 bg-transparent w-full hidden lg:block z-20"
      >
        <div className=" nav_inner bg-white rounded-[35px] px-6 py-4 flex justify-between items-center">
          ...
          <div className="menu gap-5 flex justify-center items-center">
            <a
              className=" navMenu relative text-gray-500 hover:text-purple-600 font-semibold"
              href="#about"
            >
              About
            </a>
          ...
          <div className="buttons gap-5">
            <button className="sc-8f461dda-0 buttonCenter">
              ...
              <span></span>
              <div className="sc-8f461dda-2 iWSHNC">“Combine” software</div>
              <div className="sc-8f461dda-1 buttonSide"></div>
            </button>
            <button className="sc-8f461dda-0 buttonCenter">
              <div className="sc-8f461dda-1 buttonSide"></div>
      ...
      <nav
        id="smlNav"
        className=" fixed px-7 py-4 bg-transparent w-full opacity-0 transition-all flex justify-between lg:hidden z-20"
      >
        <div className="logo flex gap-6 items-center">
          <svg
            width="56"
        ...
        <Menu onClick={()=> {setactive(true)}} className=" w-7 h-7 text-white"/>
        <div style={{background:"linear-gradient(147.35deg, rgb(35, 35, 36) -0.9%, rgb(21, 21, 22) 116.23%)"}} className={` fixed h-screen w-screen flex-col justify-center items-center gap-4 z-30 top-0 left-0 ${active? "flex":"hidden"}`}>
        <X onClick={()=> {setactive(false)}} className=" w-7 h-7 absolute top-3 right-3 text-red-600"/>
